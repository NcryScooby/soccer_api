...
const listPlayersByTeam = async (req: Request, res: Response) => {
  ...
  const SQL = `SELECT players.id, players.player_name AS name, positions.position_name AS position, teams.team_name AS team, tournaments.tournament_name AS tournament FROM players INNER JOIN teams ON players.team_id = teams.id INNER JOIN tournaments ON teams.tournament_id = tournaments.id INNER JOIN countrys ON teams.country_id = countrys.id INNER JOIN continents ON countrys.continent_id = continents.id INNER JOIN positions ON players.position_id = positions.id WHERE teams.id = ${id} ORDER BY players.player_name ASC;`;
